...
<?php } elseif ( is_page_template('template-team.php') && '-1' != get_theme_mod( 'category_team_page' ) ) { ?>
	...
	<?php $team = new WP_Query(array(
		...
		'tax_query' => array(
	        array(
		        'taxonomy' => 'category-team',
		        'field' => 'id',
		        'terms' => get_theme_mod( 'category_team_page' ),
	        ))
	    )); ?>
	    
<?php } else { ?>

	<?php $team = new WP_Query(array('post_type' => 'team', 'posts_per_page' => -1, 'suppress_filters'=>0)); ?>
			...
			<?php if ( ! empty( $post->post_excerpt ) || get_post_meta($post->ID, 'team_title', true) ) { ?>
				...
				<div class="excerpt">
					...
					<?php if ( get_post_meta($post->ID, 'team_twitter_name', true) && get_post_meta($post->ID, 'team_twitter_link', true) ) : ?>
						<p class="subtitle"><a href="<?php echo get_post_meta(get_the_ID(), 'team_twitter_link', true); ?>" rel="bookmark"><?php _e("@", 'organicthemes'); ?><?php echo get_post_meta(get_the_ID(), 'team_twitter_name', true); ?></a></p>
					<?php endif; ?>
